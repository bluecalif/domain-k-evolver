...
### 공통 필수 artifact: entity-field-matrix.json

> **Gate 판정 전 반드시 실행 — matrix 없이 Gate 선언 불가.**

trial 완료 직후, Gate 판정 전에 생성. 집계 지표(adj_yield, gap_resolution_rate)만으로는 vacant 슬롯·ku_only 슬롯을 탐지할 수 없다.
...
출력: `{bench_root}/entity-field-matrix.json` (by_category 요약 + 전체 슬롯 상태 포함)

**trail 판정 시 추가 확인 항목** (axis별 공통):
- `summary.vacant` — 이전 trial 대비 감소 여부 (S3/S4/S5a 진행 시 핵심 지표)
- `summary.gu_open` — trial 종료 시 0 이어야 정상 수렴
- `summary.ku_only` — seed KU + wildcard-GU 파생 KU 합산; 높으면 adj trigger 미작동 의심
- category별 vacant 분포 — 특정 category 집중 여부 확인 (attraction category 우선 점검)
