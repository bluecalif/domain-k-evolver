...
- [x] **0C.5 정책 확정**: expansion-policy v0.1 → v1.0 승격
  - T1 임계치 확정: deficit_ratio > 0 (required 축)
  - T2~T5 보수적 초기값 설정 (실측 미검증)
  - explore 비율: 초기 60% / 중기 50% / 수렴 40%
  - Entity Hierarchy 규칙 신설 (§7, RX-13 반영)
  - gu-bootstrap-spec.md v1.1 확정 (TBD → 확정 임계치)

- [x] **0C.6 설계 문서 통합**: design-v2.md + gu-bootstrap-spec.md 업데이트
  - design-v2: mode_node 추가, entity hierarchy 확정, State 타입 확장, Phase 1 입력 체크리스트
  - spec: Normal/Jump 이원화 상한, 확정 비율 반영, Jump Mode 체크리스트(B-2) 추가

## Current State

**Phase 0C 전체 완료** — 0C.1~0C.6 모두 완료. git commit 대기.
...
- `docs/gu-bootstrap-expansion-policy.md` — v0.1 → v1.0 승격 (0C.5)
- `docs/gu-bootstrap-spec.md` — v1.1 확정 (TBD 임계치 → 확정) (0C.5, 0C.6)
- `docs/design-v2.md` — Phase 0C 통합 (mode_node, entity hierarchy, State 타입 확장) (0C.6)

## Remaining / TODO
- [x] **0C.4 Plan Modify 작성** (`revised-plan-c3.md`) — ✅ 완료
- [x] **git commit** (0C.4 Critique + Plan Modify 완료분) — ✅ c338351
- [x] **0C.5 정책 확정** (expansion-policy v0.1 → v1.0) — ✅ 완료
- [x] **0C.6 설계 문서 통합** — ✅ 완료
- [ ] git commit (0C.1~0C.6 완료분) ← 현재 진행 중
- [ ] git push (적절한 시점)
...
- D-28: T1 임계치 = deficit_ratio > 0 확정 (required 축). T2~T5 보수적 초기값.
- D-29: explore 비율 단계별 확정: 초기 60%, 중기 50%, 수렴 40%.
- D-30: Entity Hierarchy 규칙 신설 (is_a + alias). Phase 1 schema 반영 예정.
...
## Next Action
Phase 0C 완료 git commit 후 → Phase 1 (LangGraph 자동화) 진입 또는 Cycle 3 수동 실행(선택).
