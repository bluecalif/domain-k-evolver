...
> Last Updated: 2026-03-04 (Phase 1 dev-docs 생성)
...
### Phase 1: LangGraph Core Pipeline 🔜 In Progress
- **전제**: Phase 0C 완료 ✅ (확정된 GU 전략 정책 + Axis Coverage Matrix 로직 포함)
- **상세**: `dev/active/phase1-langgraph-core/` 참조
- **Stage A**: 기반 구축 — EvolverState 타입 + I/O + Schema 검증 + Metrics (5개 task)
- **Stage B**: 노드 구현 — seed, mode, plan, collect, integrate, critique, plan_modify, hitl_gate (8개 task)
- **Stage C**: Graph 빌드 — StateGraph + 엣지 라우팅 + 단위 테스트 (3개 task)
- 총 16 tasks (S:1, M:6, L:7, XL:2)
...
| Phase 1 | A: 기반 (State, I/O, Schema, Metrics) | S~M | 5 | 🔜 Next |
| Phase 1 | B: 노드 구현 (8개: seed~hitl_gate) | M~XL | 8 | |
| Phase 1 | C: Graph 빌드 + 테스트 | M~L | 3 | |
| Phase 2 | A: 벤치 연결 | M | 3 | |
