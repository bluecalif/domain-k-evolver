...
> Status: In Progress — Phase 3 Cycle Quality Remodeling
...
- **목표**: Cycle 품질 개선 — disputed KU 문제 해결, active KU 성장률 11.5x 향상
- **근거**: Phase 2 심층 분석 결과 (docs/phase2-analysis.md) R1~R6 권고
- **Stage A**: Semantic Conflict Detection — `_detect_conflict()` LLM 기반 개선 (R1)
- **Stage B**: Dispute Resolution — disputed→active 전환 경로 + critique workflow (R2~R4)
- **Stage C**: 수렴 개선 — conflict_rate 상한 C6 + early stopping 강화 (R5~R6)
- **검증**: 10 Cycle 재실행으로 개선 효과 측정
...
| Phase 3 | A: Semantic Conflict Detection | M~L | TBD | |
| Phase 3 | B: Dispute Resolution | M~L | TBD | |
| Phase 3 | C: 수렴 개선 + 재검증 | S~L | TBD | |
